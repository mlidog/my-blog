...
# *写在最前*
<u>*因为我的这个脚本是针对CPP平台的、在2026/8/16开售的comicupg08同人展来开发的，本文仅作为我个人开发这个脚本的记录，如果你也有想法去开发一个脚本，本文仅提供思路参考，对脚本的各种功能不作保障。*</u>
&nbsp;
&nbsp;
...
上面的大部分流程都可以让codex自行完成，至于怎么在电脑上使用codex，可以看我的另一篇文章[《如何在你的电脑上简单且实惠地使用codex》](codex-application.html)。

# 三、实际操作流程
1. 下载模拟器，我用的是[MuMu模拟器](https://mumu.163.com/)（老火批了）。
2. 在MuMu模拟器上下载CPP，可以在浏览器搜索，也可以在模拟器上下个qq，用qq传过去。
3. 在电脑上下载[fiddler classic](https://www.telerik.com/download/fiddler)，下载可能会有点麻烦，所以我会在本篇附件里提供一个exe安装包。
4. 启动模拟器上的网络代理，ip为本机ip，端口指向8888。（这一步可以让codex完成，后文会提及。）
5. 在模拟器上登录CPP，填写信息，刷新几次让fiddler抓包CPP的请求。
6. 将抓包数据发给codex完成脚本。
...
2. 抓包CPP的购票端口这一步我完全是让codex自己做的（毕竟本人是计算机网络小白，根本看不懂fiddler里面大段大段的请求都是什么......），你只需要让做好购票前的准备，进入确认订单前的那个界面（最好可以随便买一张在售的票），然后去fiddler里面导出har文件，最后让codex分析har文件里面的端口请求就可以了，即：

*<small>进入这个界面</small>*
![购买界面](./cpp/buy.png)

&nbsp;
*<small>（最好买一张在售的票）</small>*

&nbsp;
*<small>保存har文件</small>*
![保存har](./cpp/savehar.png)

最后让codex自己分析就可以了。

3. 由于我们第一次抓包的token和cookie会过期，所以开售日当天要启动fiddler再抓一遍包，更新token和cookie，检查登录凭证等等。当然你只需要在开售日当天让codex再帮你走一遍抓包流程就可以了。

4. 因为此时你的模拟器启动了fiddler代理，所以他的流量都会经过fiddler的抓取，这个时候你有可能无法在模拟器玩一些网游（游戏的服务器会拒绝代理流量），所以你想在模拟器上玩游戏的时候要关掉fiddler代理。（经历来源于我抓完包后发现玩不了明日方舟......）
...
# 五、开售日当天操作
1. 让codex恢复模拟器的手动代理（如果你之前关闭过的话）。
2. 打开fiddler，重复之前在CPP的抓包流程，导出新的har文件。
3. 将har文件交给codex分析，更新token和cookie，更新优化脚本。
4. 等待开售时间，在开售前5-10分钟让codex做最后的确认，打开脚本自动运行。
![confirm](./cpp/confirm.png)
5. 开售后抢到票去支付订单！
![get](./cpp/get.png)  
...
# 六、回流票监控
codex帮我写的脚本同样还包含了回流票的监控程序。在第一轮抢票后可能会有少量的回流票，这时候脚本也能自动监控并下单，不过回流票监控脚本在我个人使用的时候触发了很多次风控，也就是不稳定，你拿到脚本后可以让codex帮你检查并优化，我不敢保证监控脚本的功能可以正常生效。。。
&nbsp;
![sk1](./cpp/1.png)
&nbsp;
![sk2](./cpp/2.png)

虽然我后面让codex优化了回流监控的脚本，但是因为没法验证，所以还是不知道会不会出问题。


## 至此，你已经学会用科技改变生活了！如果你想了解更多或者有问题可以进一步[联系我](../about.html)！

# 附件
最后我会把这个脚本文件和使用说明放到这里，你想要使用的话可以直接丢给codex，你只需要用fiddler抓包到你自己的信息就可以适配了。（当然也是给codex做）

<u>****本工具仅供学习交流，请勿用于商业倒卖***</u>
- [fiddler classic直装包下载](./cpp/FiddlerSetup.6.0.20261.7291-latest.exe)
- [脚本本体+使用说明](./cpp/cpp_grab_share.zip)
