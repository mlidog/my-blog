...
2. 抓包CPP的购票端口这一步我完全是让codex自己做的（毕竟本人是计算机网络小白，根本看不懂fiddler里面大段大段的请求都是什么......），你只需要让做好购票前的准备，进入确认订单前的那个界面（最好可以随便买一张在售的票），然后去fiddler里面保存har文件，最后让codex分析har文件里面的端口请求就可以了，即：
...
4. 因为此时你的模拟器启动了fiddler代理，所以他的流量都会经过fiddler的抓取，这个时候你有可能无法在模拟器玩一些网游（游戏的服务会拒绝代理流量），所以你想在模拟器上玩游戏的时候要关掉fiddler代理。（经历来源于我抓完包后发现玩不了明日方舟......）
...
2. 打开fiddler，重复之前在CPP的抓包流程，导入新的har文件。
...
# 六、回流票监控
codex帮我写的脚本同样还包含了回流票的监控程序。在第一轮抢票后可能会有少量的回流票，这时候脚本也能自动监控并下单，不过回流票监控脚本在我个人使用的时候触发了很多次风控，也就是不稳定，你拿到脚本后可以让codex帮你检查并优化，我不敢保证监控脚本的功能能生效。。。
&nbsp;
![sk1](./cpp/1.png)
&nbsp;
![sk2](./cpp/2.png)
...
[fiddler classic下载](./cpp/FiddlerSetup.6.0.20261.7291-latest.exe)
